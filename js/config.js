...
if (urlParams.has('demo')) {
  ...
  localStorage.setItem('controlbanquete_force_mock', isDemo ? 'true' : 'false');
  // Limpiar el query string de la barra de direcciones
  const cleanUrl = window.location.protocol + "//" + window.location.host + window.location.pathname;
  window.history.replaceState({ path: cleanUrl }, '', cleanUrl);
}

// Configuración de Firebase para Control Banquete
...
export const USE_MOCK_DATA = localStorage.getItem('controlbanquete_force_mock') === 'true' || false;
  ...
  try {
    ...
    if (response.ok) {
      ...
      return {
        ...
        useMock: localStorage.getItem('controlbanquete_force_mock') === 'true'
      };
    }
  } catch (e) {
