// Controlador Principal del Sistema de Gestión Integral Casa de Banquetes Sarahy
...
import { onAuthChange, login, logout, registerNewUser, getCurrentUser, changeUserPassword } from './auth.js';
...
function initTheme() {
  const savedTheme = localStorage.getItem('sarahy_theme') || 'dark';
  if (savedTheme === 'light') {
...
function toggleTheme() {
  ...
  localStorage.setItem('sarahy_theme', activeTheme);
...
async function loadCommonData() {
  try {
    ...
    if (demoSelector) {
      demoSelector.style.display = (localStorage.getItem('sarahy_force_mock') === 'true') ? 'block' : 'none';
    }
  } catch (err) {
...
function applyBrandingAndContract() {
  ...
  const businessName = settings.businessName || 'Sarahy';
  const businessSubtitle = settings.businessSubtitle || 'Casa de Banquetes';
  ...
  if (user) {
    ...
    if (user.role === 'superadmin') {
      links = [
        ...
        { id: 'view-operativo', label: 'Operativo', icon: '📋' },
...
function setupEventListeners() {
  ...
  document.getElementById('form-login').addEventListener('submit', async (e) => {
    ...
    try {
      ...
      if (err.message.includes('auth/configuration-not-found') || err.code === 'auth/configuration-not-found') {
        ...
        if (confirmMock) {
          localStorage.setItem('sarahy_force_mock', 'true');
  ...
  document.querySelectorAll('.tab-btn').forEach(btn => {
    btn.addEventListener('click', (e) => {
      ...
      // Desactivar todos los botones de pestañas del contenedor
      ...
      // Evento especial de renderizado si cambia pestaña
  ...
  cotForm.addEventListener('submit', async (e) => {
    ...
    const result = computePricingBreakdown(guests, venueId, photographyId, decorationId, selectedServices, baseSettings);
...
function populateCotizadorServices() {
  ...
  list.forEach(s => {
    ...
    const hasQuantity = nameLower.includes('garrafa') || nameLower.includes('caja');
...
function computePricingBreakdown(guests, venueId, photographyId, decorationId, selectedServicesIds, settings) {
  ...
  const costoAlimentacion = guests * costoAlimentacionVal;
  ...
  const total = costoMeseros + costoAlimentacion + venuePrice + photoPrice + decoPrice + servicesSum + customChargesSum;

  return {
    meseros,
    costoMeseros,
    costoAlimentacion,
    descripcionAlimentacion: settings.descripcionAlimentacion || '',
...
async function calculateLiveCotizacion() {
  ...
  const res = computePricingBreakdown(guests, venueId, photographyId, decorationId, selectedServices, settings);

  // Actualizar descripción de alimentación debajo del contador de invitados
  document.getElementById('cot-catering-desc').textContent = settings.descripcionAlimentacion || '';
  ...
  breakdownDiv.innerHTML = `
    ...
    <div class="summary-row" style="flex-direction:column; align-items:flex-start; gap:0.25rem;">
      <div style="display:flex; justify-content:space-between; width:100%;">
        <span>Banquete (Alimentación)</span>
        <span>$${res.costoAlimentacion.toLocaleString()} COP</span>
      </div>
      ${settings.descripcionAlimentacion ? `<span style="font-size:0.75rem; color:var(--text-secondary); line-height:1.2;">${settings.descripcionAlimentacion}</span>` : ''}
    </div>
    <div class="summary-row">
...
function translateEventType(type) {
  const map = { boda: 'Boda', quinces: 'Quinces', grados_otros: 'Grados/Otros', comuniones: 'Primera Comunión' };
...
async function renderAdminQuotes() {
  ...
  let filtered = quotes.filter(q => {
    ...
    // 3. Fecha de Cotización/Evento
    if (filterDateMode !== 'all' && q.date) {
      const qDate = new Date(q.date + 'T00:00:00');
      const today = new Date();
      today.setHours(0, 0, 0, 0);

      if (filterDateMode === 'today') {
        const checkDate = new Date(q.date + 'T00:00:00');
...
async function renderAdminProducts() {
  ...
  if (bizNameEl) bizNameEl.value = baseSettings.businessName || 'Sarahy';

  const bizSubEl = document.getElementById('setting-business-subtitle');
  if (bizSubEl) bizSubEl.value = baseSettings.businessSubtitle || 'Casa de Banquetes';
  ...
  function matchesFilter(cat) {
    ...
    if (filterVal === 'menu') return menuCategories.includes(cat);
    return filterVal === cat;
  }

  // Agregar salones
  if (matchesFilter('venue')) {
    prods.venues.forEach(p => addProductRow(p, tbody));
  }
  // Agregar fotos
  if (matchesFilter('photography')) {
    prods.photography.forEach(p => addProductRow(p, tbody));
  }
  // Agregar decoraciones
  if (matchesFilter('decoration')) {
    prods.decoration.forEach(p => addProductRow(p, tbody));
  }
  ...
  if (matchesFilter('service')) {
    Object.keys(prods.services).forEach(eventType => {
      prods.services[eventType].forEach(p => {
        addProductRow({ ...p, eventType }, tbody);
      });
    });
  }
  ...
  menuCategories.forEach(cat => {
    if (prods[cat] && matchesFilter(cat)) {
      prods[cat].forEach(p => addProductRow(p, tbody));
    }
  });
}

function addProductRow(p, tbody) {
  const tr = document.createElement('tr');
  const catNames = {
    venue: 'Salón',
  ...
  tr.innerHTML = `
    ...
    <td><span class="badge" style="background:rgba(255,255,255,0.05); color:white;">${catNames[p.category]}</span></td>
...
window.editProduct = async (id, category, eventType) => {
  ...
  else item = prods.services[eventType].find(p => p.id === id);
...
async function saveBaseSettings(e) {
  ...
  const businessName = document.getElementById('setting-business-name')?.value.trim() || 'Sarahy';
  const businessSubtitle = document.getElementById('setting-business-subtitle')?.value.trim() || 'Casa de Banquetes';
...
async function renderAdminUsers() {
  try {
    ...
    if (!tableContainer || !groupedContainer || !tbody) {
      console.warn("User listing container elements not found in DOM.");
      return;
    }
    ...
    if (filterRole === 'cliente') {
      // Modo agrupado por franja horaria
      ...
      filteredUsers.forEach(u => {
        // Buscar evento asociado al cliente
        const clientEvent = events.find(e => e && (e.clientId === u.uid || (e.clientEmail && e.clientEmail.toLowerCase() === u.email.toLowerCase())));
      ...
      Object.keys(groups).forEach(groupName => {
        ...
        if (items.length === 0) return; // Omitir grupos vacíos
        ...
        items.forEach(item => {
          ...
          tr.innerHTML = `
            <td>
              ...
              <span style="font-size:0.75rem; color:var(--accent-gold);">${e ? translateEventType(e.eventType) + ' | ' + e.date + ' ' + (e.time || '') : 'Sin evento programado'}</span>
            </td>
            <td>${u.email}</td>
      ...
      filteredUsers.forEach(u => {
        const clientEvent = events.find(e => e && (e.clientId === u.uid || (e.clientEmail && e.clientEmail.toLowerCase() === u.email.toLowerCase())));
        const photoStatusHtml = clientEvent ? `<span class="badge ${getPhotoStatusBadgeClass(clientEvent.photoStatus)}" style="font-size:0.7rem; padding:0.1rem 0.4rem; text-transform:uppercase;">📸 ${translatePhotoStatus(clientEvent.photoStatus)}</span>` : '-';
    ...
    console.error("Error in renderAdminUsers:", err);
    alert("Error en renderAdminUsers: " + err.message + "\n" + err.stack);
  }
}
...
function openProductModal(prod = null) {
  ...
  if (prod) {
    ...
    if (prod.category === 'service') {
      ...
      document.getElementById('p-event-type').value = prod.eventType || 'boda';
    } else {
...
async function saveAdminProduct(e) {
  ...
  const eventType = category === 'service' ? document.getElementById('p-event-type').value : null;

  const product = { name, category, price, description, infoUrl };
...
function addProductShoppingRow(name = '', qty = '', unit = '', scale = false, supplier = '', purchaseInstructions = '') {
  ...
  div.innerHTML = `
    ...
    <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 0.5rem;">
      <input type="text" class="prod-item-supplier" value="${supplier}" placeholder="Proveedor (Ej: Distribuidora X)" style="padding:0.4rem; font-size:0.8rem; border-radius:6px; border:1px solid var(--border-color); background:var(--bg-secondary); color:var(--text-primary);">
...
async function openEventModal(ev = null) {
  ...
  users.filter(u => u.role === 'cliente').forEach(c => {
    clientSelect.innerHTML += `<option value="${c.uid}">${c.name} (${c.email})</option>`;
  });
  ...
  // Cargar Fotografía y Decoración dropdowns
...
async function saveInventoryItem(e) {
  ...
  const item = { name, quantity, unit, category, minStock };
...
async function calculateAutomatedPurchase() {
  ...
  if (!ev) return;
  if (!ev.menu) {
    alert("El cliente no ha seleccionado ninguna opción en su menú todavía.");
    return;
  }
  ...
  Object.keys(consolidated).forEach(key => {
    ...
    tbody.innerHTML += `
      <tr>
        ...
        <td style="color:${needsPurchase ? 'var(--warning)' : 'var(--success)'}; font-weight:700;">
          ${formattedDeficit} ${item.unit}
        </td>
        <td>
          <span class="badge ${needsPurchase ? 'badge-pendiente_pago' : 'badge-confirmada'}">
            ${needsPurchase ? 'Comprar' : 'Stock OK'}
          </span>
        </td>
      </tr>
    `;
...
async function loadOperativoDetails() {
  ...
  renderRoleRequirements(ev);
}

function renderRoleRequirements(ev) {
  ...
  if (currentRole === 'logistica' || currentRole === 'superadmin') {
    ...
    selectedS.forEach(sid => {
      ...
      Object.keys(allProducts.services).forEach(key => {
        ...
        if (match && (match.name.includes('Mariachi') || match.name.includes('Papayera') || match.name.includes('Recreacion'))) {
...
async function downloadQuoteById(quoteId) {
  try {
    ...
    const baseSettings = await DB.getSettings();
    const breakdown = computePricingBreakdown(q.guests, q.venueId, q.photographyId, q.decorationId, q.selectedServices || [], baseSettings);
...
async function triggerDownloadLiveQuotation() {
  ...
  const baseSettings = await DB.getSettings();
  const breakdown = computePricingBreakdown(guests, venueId, photographyId, decorationId, selectedServices, baseSettings);
...
function downloadQuotationPDF(data) {
  ...
  const businessName = settingsObj.businessName || 'Sarahy';
  const businessSubtitle = settingsObj.businessSubtitle || 'Casa de Banquetes';
  ...
  // Encontrar salón
  const venue = allProducts.venues.find(v => v.id === data.breakdown.venueId || v.id === data.breakdown.venueName || v.name === data.breakdown.venueName);
  ...
  if (data.breakdown.photoPrice > 0) {
    const photo = allProducts.photography.find(p => p.id === data.breakdown.photoId || p.price === data.breakdown.photoPrice);
    detailsList.push({ 
  ...
  if (data.breakdown.decoPrice > 0) {
    const deco = allProducts.decoration.find(d => d.id === data.breakdown.decorationId || d.price === data.breakdown.decoPrice);
    detailsList.push({ 
  ...
  // Añadir servicios individuales
  const selectedCards = document.querySelectorAll('#cot-services-grid .select-card.selected');
  if (selectedCards.length > 0 && data.id.startsWith('TEMP_')) {
    selectedCards.forEach(card => {
      const name = card.querySelector('.select-card-name').textContent;
      const priceVal = parseInt(card.getAttribute('data-price'));
      const idVal = card.getAttribute('data-id');
      const match = allServices.find(item => item.id === idVal);
      detailsList.push({ 
        name, 
        price: priceVal,
    ...
    // Si viene desde admin quotes, buscamos por los IDs en data.breakdown
    // los cuales corresponden a los servicios seleccionados guardados en Firestore
    const selectedIds = data.breakdown.selectedServices || [];
    selectedIds.forEach(sid => {
      const match = allServices.find(item => item.id === sid);
      if (match) {
        detailsList.push({ 
          name: match.name, 
          price: match.price,
  ...
  if (data.breakdown.customChargesList && Array.isArray(data.breakdown.customChargesList)) {
    data.breakdown.customChargesList.forEach(cc => {
      customSettingsHtml += `
        <tr>
          <td>
            <strong>${cc.label}</strong><br>
            <span style="font-size:0.8rem; color:#718096;">Cargo operativo base${cc.type === 'per_person' ? ' (calculado por persona)' : ''}</span>
          </td>
          <td style="text-align: right; font-weight: 500;">$${cc.totalCost.toLocaleString()}</td>
        </tr>
      `;
    });
  }
  ...
  const html = `
    ...
    <head>
      ...
      <style>
        body { font-family: 'Helvetica Neue', Helvetica, Arial, sans-serif; color: #1a202c; margin: 40px; line-height: 1.5; }
        .header { display: flex; justify-content: space-between; border-bottom: 3px double #d4af37; padding-bottom: 20px; margin-bottom: 30px; }
        .logo-title { font-family: Georgia, serif; color: #1a202c; font-size: 28px; margin: 0; font-weight: bold; letter-spacing: 1px; }
        .logo-sub { font-size: 11px; text-transform: uppercase; color: #d4af37; letter-spacing: 3px; font-weight: bold; }
        .info-grid { display: flex; justify-content: space-between; margin-bottom: 30px; font-size: 14px; }
        .info-col { width: 48%; background: #f7fafc; padding: 15px; border-radius: 8px; border: 1px solid #edf2f7; }
        .info-title { font-weight: bold; color: #1a202c; border-bottom: 2px solid #d4af37; padding-bottom: 5px; margin-bottom: 10px; text-transform: uppercase; font-size: 11px; letter-spacing: 1px; }
        table { width: 100%; border-collapse: collapse; margin-bottom: 30px; font-size: 14px; }
        th { background: #1a202c; text-align: left; padding: 12px; border-bottom: 2px solid #ddd; color: #fff; font-weight: 500; }
        td { padding: 12px; border-bottom: 1px solid #e2e8f0; }
        .total-box { display: flex; flex-direction: column; align-items: flex-end; font-size: 14px; }
        .total-row { display: flex; justify-content: space-between; width: 320px; padding: 6px 0; border-bottom: 1px dashed #e2e8f0; }
        .total-row.final { border-top: 2px solid #d4af37; border-bottom: none; padding-top: 12px; margin-top: 8px; font-size: 20px; font-weight: bold; color: #d4af37; }
        .footer { border-top: 1px solid #edf2f7; margin-top: 60px; padding-top: 20px; text-align: center; font-size: 11px; color: #718096; line-height: 1.6; }
      </style>
    </head>
    <body>
      ...
      <div class="info-grid">
        <div class="info-col">
          ...
          <strong>Correo Electrónico:</strong> ${data.clientEmail}<br>
          <strong>Teléfono / WhatsApp:</strong> ${data.clientPhone}
        </div>
        <div class="info-col">
          <div class="info-title">Detalles del Evento</div>
          <strong>Tipo de Evento:</strong> ${translateEventType(data.eventType)}<br>
          <strong>Fecha Programada:</strong> ${data.date}<br>
          <strong>Número de Invitados:</strong> ${data.guests} personas
        </div>
      </div>
      ...
      <table>
        ...
        <tbody>
          <tr>
            <td>
              <strong>Banquete Principal (Alimentación)</strong><br>
              <span style="font-size:0.8rem; color:#718096;">${data.breakdown.descripcionAlimentacion || `Menú tipo buffet a elección para ${data.guests} invitados`}</span>
            </td>
            <td style="text-align: right; font-weight: 500;">$${data.breakdown.costoAlimentacion.toLocaleString()}</td>
          </tr>
          <tr>
            <td>
              <strong>Personal de Servicio (Meseros)</strong><br>
              <span style="font-size:0.8rem; color:#718096;">${data.breakdown.meseros} meseros calificados incluidos para la atención del evento</span>
            </td>
            <td style="text-align: right; font-weight: 500;">$${data.breakdown.costoMeseros.toLocaleString()}</td>
          </tr>
          ${detailsList.map(item => `
            <tr>
              <td>
                <strong>${item.name}</strong><br>
                <span style="font-size:0.8rem; color:#718096;">${item.desc || 'Servicio integrado según catálogo'}</span>
              </td>
      ...
      <div class="total-box">
        <div class="total-row">
          <span>Servicio Base (Comida + Meseros):</span>
